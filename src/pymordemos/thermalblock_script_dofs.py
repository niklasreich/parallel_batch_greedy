...
def main(
    ...
    grid: int = Argument(..., help='Number of grid points per dim.')
):
    """Thermalblock demo."""
    ...
    if batchsize==1:
        filestring = f'thermalblock_{xblocks}x{yblocks}_B1_dofs{grid}.pkl'
    else:
        filestring = f'thermalblock_{xblocks}x{yblocks}_N{len(pool)}_dofs{grid}.pkl'
